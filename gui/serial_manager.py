...
class SerialManager:
    ...
    def read_pipeline_packet(self) -> Tuple[PipelineStatus, object]:
        ...
        #------------------------------------
        while True:
            print("Waiting for Pipeline Dump Alert (0xDA)...")
            b = self.ser.read(1)
            if b and ord(b) == CMD_DUMP_ALERT:
                print("🚨 Pipeline Dump Alert Received.")
                break
        ...
        print(f"📦 {dump_mode.name} Pipeline Packet Incoming...")

        #------------------------------------
        # 3. Read 50 Words (RegFile + Pipeline)
        #------------------------------------
        raw_pipe_data = self.ser.read(200)
        print("📥 Pipeline Data Received.")
        pipe_words = unpack_words(raw_pipe_data, 50)
        self.log_raw(pipe_words, "PIPELINE_PACKET")

        # Unpack them into PipelineStatus data class
        pipeline_status = PipelineStatus.unpack(dump_mode, pipe_words)
        
        #------------------------------------
        # 4. Read Memory Header
        #------------------------------------
        raw_mem_flag = self.ser.read(4)
        self.log_raw(unpack_words(raw_mem_flag, 1), "ONE_WORD_MEMORY_PAD")
        ...
        if dump_mode == MemoryDumpMode.SNOOP_BASED:
            ...
            raw_mem_flag = self.ser.read(4)
            self.log_raw(unpack_words(raw_mem_flag, 1), "MEMORY_FLAG")

            mem_flag_word = unpack_words(raw_mem_flag, 1)[0]
            print(mem_flag_word)  #delete, temp
            ...
            self.log_raw(unpack_words(raw_address_range, 2), "MEMORY_ADDRESS_RANGE")

            address_range = unpack_words(raw_address_range, 2)
            print("📊 Reading Memory Continuous/Range Data...")
            print(f"ℹ️ Memory Range Minimum Address: 0x{address_range[0]:08X}")
            print(f"ℹ️ Memory Range Maximum Address: 0x{address_range[1]:08X}")

            if address_range[0] == 0xFFFFFFFF and address_range[1] == 0x00000000:
                print("ℹ️ No Memory Updates to patch in Continuous/Range Mode.")

                return pipeline_status, MemPatch.unpack([0xFFFFFFFF, 0x00000000, 0x00000000])
            
            print(f"ℹ️ Calculating Memory Payload Size...")
            ...
            print(f"ℹ️ Range: {address_range[0]:08x} - {address_range[1]:08x}")
            print(f"ℹ️ Expecting {num_words} words ({total_bytes_expected} bytes) of memory payload...")
            ...
            self.log_raw(mem_payload_words, "MEMORY_PAYLOAD")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Check connection or COM port.")
