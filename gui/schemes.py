...
from instructions import BaseInstruction, InstructionFactory
...
class MemoryDumpMode(Enum):
    ...
    def __str__(self):
        if self == MemoryDumpMode.RANGE_BASED:
            return "Memory was scanned using Memory Range from the Memory Range Tracker"
        elif self == MemoryDumpMode.SNOOP_BASED:
            return "Memory was scanned as single diff from the last Memory Dump"
        else:
            return "UNKNOWN"
